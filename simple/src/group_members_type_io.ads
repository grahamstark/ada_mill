--
-- Created by ada_generator.py on 2013-06-11 17:03:46.903918
-- 
with Simple_Pg_Data;
with DB_Commons;
with Base_Types;
...
package Group_Members_Type_IO is
   ...
   --
   function Retrieve_By_PK( Group_Name : Unbounded_String; User_Id : Integer; connection : Database_Connection := Null_Database_Connection ) return Simple_Pg_Data.Group_Members_Type;
   
   --
   -- Retrieves a list of Simple_Pg_Data.Group_Members_Type matching the criteria, or throws an exception
   --
   function Retrieve( c : d.Criteria; connection : Database_Connection := Null_Database_Connection ) return Simple_Pg_Data.Group_Members_Type_List.Vector;
   
   --
   -- Retrieves a list of Simple_Pg_Data.Group_Members_Type retrived by the sql string, or throws an exception
   --
   function Retrieve( sqlstr : String; connection : Database_Connection := Null_Database_Connection ) return Simple_Pg_Data.Group_Members_Type_List.Vector;
   
   --
   -- Save the given record, overwriting if it exists and overwrite is true, 
   -- otherwise throws DB_Exception exception. 
   --
   procedure Save( group_members : Simple_Pg_Data.Group_Members_Type; overwrite : Boolean := True; connection : Database_Connection := Null_Database_Connection );
   
   --
   -- Delete the given record. Throws DB_Exception exception. Sets value to Simple_Pg_Data.Null_Group_Members_Type
   --
   procedure Delete( group_members : in out Simple_Pg_Data.Group_Members_Type; connection : Database_Connection := Null_Database_Connection );
   --
